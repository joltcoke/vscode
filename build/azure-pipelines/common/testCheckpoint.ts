...
const testIds = ['unit-electron', 'unit-node', 'unit-browser-chromium'] as const;
...
/**
 * A deliberately opt-in Linux x64 pilot. Only published artifacts from this run
 * count as checkpoints; local files and attempt numbers never indicate a hit.
 */
export async function testCheckpoint(
	...
	if (!(command === 'restore' && args.length === 1)
		&& !(command === 'record' && args.length === 2 && testIds.some(id => id === testId))) {
		throw new Error('Usage: node testCheckpoint.ts restore | record <unit-electron|unit-node|unit-browser-chromium>');
	}

	if (env.AGENT_OS !== 'Linux' || env.VSCODE_ARCH !== 'x64') {
		throw new Error('Test checkpoints are only supported by the Linux x64 pilot');
	}
	...
	const artifactName = (id: string) => `test-pass-v1-${stage}-${job}-linux-x64-${id}`;
